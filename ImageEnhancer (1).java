...
public class ImageEnhancer extends Component implements ActionListener {
 ...
 String startingImage = "AYPE-Rainier-Vista.jpg";
  ...
  biFiltered = threshold_op.filter(biWorking, null);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        // Undo/redo logic & history management
        Object src = e.getSource();

        // Handle Undo click
        if (src == undoItem) {
            if (!undoStack.isEmpty()) {
                // Move current to Redo, bring top of Undo back
                redoStack.push(copyImage(biWorking));
                biFiltered = undoStack.pop();
            } else {
                biFiltered = biWorking; // nothing to undo
            }
            // Update menu enable states
        ...
        // Handle Redo click
            ...
            if (!redoStack.isEmpty()) {
                // Move current to Undo, bring top of Redo back
                undoStack.push(copyImage(biWorking));
                biFiltered = redoStack.pop();
            } else {
                biFiltered = biWorking; // nothing to redo
            }
            // Update menu enable states
            undoItem.setEnabled(!undoStack.isEmpty());
            redoItem.setEnabled(!redoStack.isEmpty());
        }
        // Before applying any new edit, save a snapshot to Undo and clear Redo
        else if (src == blurItem || src == sharpenItem || src == darkenItem
              || src == photoNegItem || src == thresholdItem) {
            // Save current for Undo
            undoStack.push(copyImage(biWorking));
            // Clear all Redo history because we are branching
            while (!redoStack.isEmpty()) {
                try { redoStack.pop(); } catch (Exception ignore) {}
            }
 ...
 public BufferedImage getBufferedImage() { // For use by the autograder
  return biWorking;
    ...
    public void run() {
        JFrame f = new JFrame("Image Enhancer WITH Undo AND Redo by Catherine Lin"); // Students should update this.
